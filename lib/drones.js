...
drones._getUserApp = function(options, fn, callback) {
  var self = this;
...
drones._changeState = function(options, fn, state) {
  ...
  this._getUserApp(options, fn, function(options) {
...
drones.logs = function(options, fn) {
  ...
  this._getUserApp(options, fn, function(options) {
    ...
    self.parent.request(path, 'get', params, function(err, result) {
      if (err)
        return fn(err);

      var response = {raw: result, stream: options.stream};

      //format logs if not streaming
      if (!options.stream) {
        var message = [['Time', 'Message', 'Type']];

        if (result && result.push && result.length > 0) {
          result.forEach(function(l) {
            message.push([l.ts, l.msg, l.type]);
          });
        }
